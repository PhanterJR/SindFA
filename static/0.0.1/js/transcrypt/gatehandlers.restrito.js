// Transcrypt'ed from Python, 2020-02-28 04:51:56
...
var __name__ = 'gatehandlers.restrito';
...
export var Index =  __class__ ('Index', [handler.GateHandler], {
	...
	get start () {return __get__ (this, function (self) {
		var html = CONCATENATE (DIV (DIV (DIV (DIV ('RESTRITO', __kwargtrans__ ({_class: 'phanterpwa-breadcrumb'})), __kwargtrans__ ({_class: 'phanterpwa-breadcrumb-wrapper'})), __kwargtrans__ ({_class: 'p-container'})), __kwargtrans__ ({_class: 'title_page_container card'})), DIV (DIV (DIV (DIV (DIV (I (__kwargtrans__ (dict ({'_class': 'fas fa-edit promo-icon'}))), H2 ('ASSOCIADOS', __kwargtrans__ ({_class: 'promo-title'})), __kwargtrans__ (dict ({'_class': 'link', '_phanterpwa-way': 'socios'}))), DIV ('Aqui permite ao administrador visualizar, adicionar, editar o cadastro de associados.', __kwargtrans__ ({_class: 'promo-content'})), __kwargtrans__ (dict ({'_class': 'promo-container'}))), __kwargtrans__ ({_class: 'p-col w1p100 w3p50 w4p25'})), __kwargtrans__ ({_class: 'p-row card e-padding_20'})), __kwargtrans__ ({_class: 'phanterpwa-container p-container'})));
...
//# sourceMappingURL=gatehandlers.restrito.map