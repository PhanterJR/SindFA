// Transcrypt'ed from Python, 2020-02-20 14:42:20
...
export var Datepickers =  __class__ ('Datepickers', [object], {
	...
	get __init__ () {return __get__ (this, function (self, target_selector) {
		...
		if ($ (self.target_selector).length > 0) {
			if ($ (self.target_selector) [0].hasAttribute ('phanterpwa-datetimepicker-iso')) {
				self.selected_date = new Date ($ (self.target_selector).attr ('phanterpwa-datetimepicker-iso'));
		...
		if (__in__ ('id_input_target', parameters)) {
			...
			if ($ (self.id_input_target).length > 0) {
				if (validations.check_datetime ($ (self.id_input_target).val ().py_replace ('_', ''), self.format, self.date_type)) {
					self._read_formated ($ (self.id_input_target).val ().py_replace ('_', ''));
					var iso_format = self._apply_format (self.format);
	...
	get _onChoice () {return __get__ (this, function (self) {
		...
		if (self.onChoice !== null && self.onChoice !== undefined) {
			...
			$ (self.target_selector).attr ('phanterpwa-datetimepicker-iso', iso_format);
			if ($ (self.id_input_target).length > 0) {
				$ (self.id_input_target).val (self._apply_format (self.format));
			}
			if (self.debug) {
