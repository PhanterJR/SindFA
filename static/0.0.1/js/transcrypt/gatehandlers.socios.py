        ...
        if self.current_hash is not json.hash:
            ...
            if socios.data is not js_undefined:
                for x in socios.data:
                    ...
                    __pragma__("nojsiter")
                    vars_serialized = jQuery.param(vars_socio)
                    table.append(
                        XTRD(
                            ...
                            widgets.MenuBox(
                                ...
                                xml_menu=UL(
                                    ...
                                    LI("Visualizar", **{
                                        ...
                                        "_phanterpwa-way": "socios/{0}/visualizar?{1}".format(x.id, vars_serialized)
                                    }),
...
class Socio(helpers.XmlConstructor):
    ...
    def visualizar(self, id_socio, params):
        url_image = "{0}/api/socios/{1}/image".format(
        ...
        self._carteira = DIV(
            ...
            _class="visualizar_socio_container"
        )
        self._carteira.html_to("#content-socios")
        window.PhanterPWA.LOAD(**{
            "args": ["loads", "svg_logo.html"],
            "onComplete": lambda data: jQuery("#content-socios").find(".carteira-logo").html(data),
        })
        url = "{0}/associado/{1}".format(
            window.PhanterPWA.CONFIG["CONFIGJS"]["api_server_address"],
            qrcode
        )
        qrcode = __new__(QRCode(jQuery("#content-socios").find(".carteira-qrcode")[0], {
            "text": url,
            "width": 125,
            "height": 125,
            "colorDark": "#000000",
